...
const Ebooks = () => {
  ...
  const categories = ["Todos", "Arquitetura", "Design de Interiores", "Marcenaria", "Sustentabilidade", "Tendências", "Ferramentas"];
  ...
  const filteredEbooks = ebooks.filter(ebook => {
    const matchesSearch = ebook.title.toLowerCase().includes(searchTerm.toLowerCase()) || ebook.author.toLowerCase().includes(searchTerm.toLowerCase()) || (ebook.description || '').toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const matchesType = selectedType === "Todos" || selectedType === "Gratuito" && ebookType === "free" || selectedType === "Premium" && ebookType === "premium";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-light text-gray-900 mb-4">
              ...
              {selectedCategory !== "Todos" && <span className="block text-2xl font-normal text-gray-600 mt-2">
                  {selectedCategory}
                </span>}
            </h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              {selectedCategory !== "Todos" ? `Explore nossa coleção de e-books especializados em ${selectedCategory}` : "Acesse nossa coleção exclusiva de e-books especializados em arquitetura, design e marcenaria"}
            </p>
          </div>
          ...
          <div className="mb-8 space-y-4">
            <div className="relative max-w-md mx-auto">
              ...
              <Input placeholder="Buscar por título, autor ou tema..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
            ...
            <div className="flex flex-wrap gap-4 justify-center items-center">
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map(category => <button key={category} onClick={() => setSelectedCategory(category)} className={`px-3 py-1 rounded-full text-sm border transition-colors ${selectedCategory === category ? 'bg-gray-900 text-white border-gray-900' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'}`}>
                    {category}
                  </button>)}
              </div>

              <div className="flex gap-2">
                {types.map(type => <button key={type} onClick={() => setSelectedType(type)} className={`px-3 py-1 rounded-full text-sm border transition-colors ${selectedType === type ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'}`}>
                    {type}
                  </button>)}
              </div>
            </div>
                  ...
                  <Card className={`group hover:shadow-lg transition-all duration-300 ${ebook.featured ? 'ring-2 ring-gray-900 ring-opacity-10' : ''}`}>
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex items-center justify-between mb-2">
                        <Badge variant="outline" className={`text-xs ${getCategoryColor(ebook.category)}`}>
                          {ebook.category}
                        </Badge>
